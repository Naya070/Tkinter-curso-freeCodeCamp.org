...
#Creamos una etiqueta en donde se visualizarán las imágene
# s.
#Las imágenes deben ser montadas sobre un widget o no se verán.
my_label= Label(image=my_img1, width=1000, height=450) #Estamos en la imagen 1 al iniciar.
...
def forward(image_number):
    ...
    my_label = Label(image= image_list[image_number-1], width=1000, height=450) #Esta será la nueva imagen.
...
def back(image_number):
     ...
     my_label = Label(image= image_list[image_number-1], width=1000, height=450)
     button_forward = Button(root, text= ">>", command= lambda: forward(image_number+1))
     button_back = Button(root, text= "<<", command= lambda: back(image_number-1))
